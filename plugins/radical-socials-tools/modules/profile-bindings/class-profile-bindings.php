 ...
 * Sources registered:
 *
 *   - `radical-socials/profile-handle` → "@user@host" for the AP blog
 ...
 *
 *   - `radical-socials/profile-bio` → the site owner's bio (the
 ...
 *           "content": { "source": "radical-socials/profile-handle" }
 *       }}} -->
 *   <p>@placeholder@example.com</p>
 *   <!-- /wp:paragraph -->
 *
 *   <!-- wp:paragraph {"metadata":{"bindings":{
 *           "content": { "source": "radical-socials/profile-bio" }
 *       }}} -->
 *   <p>Placeholder bio</p>
 *   <!-- /wp:paragraph -->
 *
...
class Radical_Socials_Profile_Bindings {
	...
	const HANDLE_SOURCE = 'radical-socials/profile-handle';
	const BIO_SOURCE    = 'radical-socials/profile-bio';
	...
	public static function register_sources(): void {
		...
		register_block_bindings_source(
			self::HANDLE_SOURCE,
			[
				'label'              => __( 'Fediverse handle', 'radical-socials-tools' ),
		...
		register_block_bindings_source(
			self::BIO_SOURCE,
			[
				'label'              => __( 'Bio', 'radical-socials-tools' ),
	...
	private static function resolve_user_id(): int {
		...
		/**
		 * Filters which user's profile the `radical-socials/profile-*`
		 * binding sources read from. Return 0 to render nothing.
		 *
		 * @param int $user_id Default user id (first administrator).
		 */
		return (int) apply_filters( 'radical_socials_profile_user_id', $default );
	}
}
