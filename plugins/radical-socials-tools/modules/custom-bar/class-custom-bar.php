...
class Radical_Socials_Custom_Bar {
	...
	public static function get_links(): array {
		$links = [
			[
				'id'    => 'home',
				'label' => __( 'Home', 'radical-socials-tools' ),
				'url'   => home_url( '/' ),
		...
		if ( is_user_logged_in() && current_user_can( 'publish_posts' ) ) {
			$links[] = [
				'id'    => 'create',
				'label' => __( 'Create', 'radical-socials-tools' ),
				'url'   => '#',
		...
		if ( current_user_can( 'moderate_comments' ) ) {
			$links[] = [
				'id'    => 'comments',
				'label' => __( 'Comments', 'radical-socials-tools' ),
		...
		if ( current_user_can( 'manage_options' ) ) {
			$links[] = [
				'id'    => 'profile',
				'label' => __( 'Profile', 'radical-socials-tools' ),
	...
	public static function render(): void {
		...
		$avatar    = $avatar_id ? wp_get_attachment_image(
			...
			[
				'class' => 'rs-bar-avatar',
				'alt'   => esc_attr__( 'Profile', 'radical-socials-tools' ),
			]
		) : '';
		$avatar    = $avatar ?: get_avatar( $user->ID, self::H, '', esc_attr__( 'Profile', 'radical-socials-tools' ), [ 'class' => 'rs-bar-avatar' ] );
		$pending   = (int) wp_count_comments()->moderated;
		$links     = self::get_links();
		?>
		<nav id="rs-bar" aria-label="<?php esc_attr_e( 'Site navigation', 'radical-socials-tools' ); ?>">
			<ul>
				<?php foreach ( $links as $link ) : ?>
					<?php
					$aria = 'comments' === $link['id'] && $pending > 0
						/* translators: %d: number of pending comments awaiting moderation. */
						? sprintf( __( 'Comments — %d pending', 'radical-socials-tools' ), $pending )
						: $link['label'];
					?>
					<li>
