...
class Radical_Socials_Following {
	...
	public static function add_cron_schedules( array $schedules ): array {
		$schedules[ self::FETCH_SCHEDULE ] = [
			...
			'display'  => __( 'Every 15 minutes', 'radical-socials-tools' ),
		];
		return $schedules;
	}
	...
	public static function register_cpt(): void {
		register_post_type(
			...
			[
				...
				'labels'              => [
					'name'          => __( 'Following', 'radical-socials-tools' ),
					'singular_name' => __( 'Feed Item', 'radical-socials-tools' ),
				],
			]
		);
	}
	...
	public static function register_taxonomy(): void {
		...
		register_taxonomy( 'rs_source', 'rs_feed_item', array_merge( $shared, [
			'labels' => [
				'name'          => __( 'Feed Sources', 'radical-socials-tools' ),
				'singular_name' => __( 'Feed Source', 'radical-socials-tools' ),
			],
		] ) );

		register_taxonomy( 'rs_feed_type', 'rs_feed_item', array_merge( $shared, [
			'labels' => [
				'name'          => __( 'Feed Types', 'radical-socials-tools' ),
				'singular_name' => __( 'Feed Type', 'radical-socials-tools' ),
			],
		] ) );

		register_taxonomy( 'rs_feed_category', 'rs_feed_item', array_merge( $shared, [
			'labels' => [
				'name'          => __( 'Feed Categories', 'radical-socials-tools' ),
				'singular_name' => __( 'Feed Category', 'radical-socials-tools' ),
			],
		] ) );
	}
	...
	public static function gate_activitypub_blocks( string $block_content, array $block ): string {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			return sprintf(
				...
				esc_html( sprintf( /* translators: %s: block name (e.g. activitypub/reactions) */ __( '%s needs the ActivityPub plugin.', 'radical-socials-tools' ), $name ) ),
				esc_url( $install_url ),
				esc_html__( 'Install it', 'radical-socials-tools' )
			);
		}
		return '';
	}
	...
	public static function wrap_following_query( string $html, array $block ): string {
		...
		$new_posts_forms = _n_noop( '%d new post', '%d new posts', 'radical-socials-tools' );
		wp_interactivity_state( 'radical-socials/following', [
			...
			'refreshing'           => false,
			'pulling'              => false,
			'refreshLabel'         => __( 'Refresh feed', 'radical-socials-tools' ),
			'refreshingLabel'      => __( 'Refreshing…', 'radical-socials-tools' ),
			'noNewPostsLabel'      => __( 'No new posts', 'radical-socials-tools' ),
			'stillRefreshingLabel' => __( 'Still refreshing', 'radical-socials-tools' ),
			'newPostLabel'         => translate_nooped_plural( $new_posts_forms, 1, 'radical-socials-tools' ),
			'newPostsLabel'        => translate_nooped_plural( $new_posts_forms, 2, 'radical-socials-tools' ),
		] );
		...
		$pull_indicator = ( is_user_logged_in() && $is_following )
			? '<div class="rs-refresh-bar" data-wp-class--rs-pull-refreshing="state.refreshing">'
				. '<button class="rs-refresh-btn" data-wp-on--click="actions.refresh" data-wp-bind--disabled="state.refreshing">'
					...
					. '<span class="rs-refresh-label">' . esc_html__( 'Refresh feed', 'radical-socials-tools' ) . '</span>'
				. '</button>'
	...
	private static function last_refreshed_label( int $last_fetched ): string {
		if ( ! $last_fetched ) {
			return __( 'Not refreshed yet', 'radical-socials-tools' );
		}

		$elapsed = time() - $last_fetched;
		if ( $elapsed < MINUTE_IN_SECONDS ) {
			return __( 'Last refreshed just now', 'radical-socials-tools' );
		}

		return sprintf(
			/* translators: %s: Human-readable elapsed time, for example "8 minutes". */
			__( 'Last refreshed %s ago', 'radical-socials-tools' ),
