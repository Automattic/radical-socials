		...
		foreach ( $actors as $post ) {
			...
			if ( ! $outbox_url ) {
				self::record_actor_health( $post->ID, [
					'status'     => 'failed',
					'error'      => __( 'Could not discover the actor\'s outbox URL (account may not exist on this server).', 'radical-socials-tools' ),
			...
			if ( ! $page ) {
				self::record_actor_health( $post->ID, [
					'status'     => 'failed',
					'error'      => __( 'The remote server returned an empty or malformed outbox page.', 'radical-socials-tools' ),
					'elapsed_ms' => (int) round( ( microtime( true ) - $poll_started ) * 1000 ),
				] );
				continue;
			}
	...
	private static function render_boost_context( string $booster_actor_url, string $booster_display ): string {
		...
		$label = '' !== $booster_display
			? sprintf(
				...
				__( 'Boosted by %s', 'radical-socials-tools' ),
				'<code>@' . esc_html( $booster_display ) . '</code>'
			)
			: esc_html__( 'Boosted', 'radical-socials-tools' );
	...
	private static function render_reply_context( array $object ): string {
		...
		$label = '' !== $display
			? sprintf(
				...
				__( '↪️ In reply to %s', 'radical-socials-tools' ),
				'<code>' . esc_html( $display ) . '</code>'
			)
			: esc_html__( '↪️ In reply to a post', 'radical-socials-tools' );
