...
class Radical_Socials_WPCOM_Proxy {
	...
	public static function handle_callback( WP_REST_Request $request ): void {
		...
		if ( ! self::looks_like_state( $state ) ) {
			wp_die( esc_html__( 'Bad OAuth state', 'radical-socials-tools' ), '', [ 'response' => 400 ] );
		}

		$origin = get_transient( self::STATE_PREFIX . $state );
		if ( ! $origin ) {
			wp_die( esc_html__( 'Unknown or expired OAuth state — start the connection again from your site.', 'radical-socials-tools' ), '', [ 'response' => 400 ] );
		}
