...
function heckl_render_social_menu( $attributes, $content, $block ): string {
	...
	foreach ( $links as $link ) {
		...
		$list_items .= sprintf(
			'<li class="wp-block-heckl-social-menu__item is-heckl-link-%1$s">'
				. '<a href="%2$s"%3$s>'
					. '<span class="dashicons %4$s" aria-hidden="true"></span>'
					. '<span class="wp-block-heckl-social-menu__label">%5$s</span>'
				. '</a>'
			. '</li>',
			esc_attr( $link['id'] ),
			esc_url( $link['url'] ),
