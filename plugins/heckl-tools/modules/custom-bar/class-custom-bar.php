 ...
 * removing the require_once in heckl.php.
 *
...
class Radical_Socials_Custom_Bar {
	...
	public static function register_blocks(): void {
		...
		register_block_type(
			...
			[ 'render_callback' => 'heckl_render_social_menu' ]
		);
	}
	...
	public static function localize_block_editor(): void {
		$handle = generate_block_asset_handle( 'heckl/social-menu', 'editorScript' );
	 ...
	 * bar and the heckl/nav-links block.
	 *
	 * Each item:
	...
	public static function get_links(): array {
		...
		if ( current_user_can( 'manage_options' ) ) {
			$links[] = [
				'id'    => 'profile',
				'label' => __( 'Profile', 'heckl-tools' ),
				'url'   => admin_url( 'admin.php?page=heckl-settings' ),
				'icon'  => 'dashicons-admin-users',
			];
		}

		return apply_filters( 'heckl_nav_links', $links );
	}
			...
			<ul>
				<?php foreach ( $links as $link ) : ?>
					...
					$aria = 'comments' === $link['id'] && $pending > 0
						...
						? sprintf( __( 'Comments — %d pending', 'heckl-tools' ), $pending )
						: $link['label'];
					?>
					<li>
