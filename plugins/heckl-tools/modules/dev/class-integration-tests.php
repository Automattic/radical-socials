...
class Radical_Socials_Integration_Tests {
	...
	private function execute(): void {
		...
		WP_CLI::success( 'Heckl integration checks passed.' );
	}
	...
	private function test_rss_canonical_urls(): void {
		...
		$this->with_http_mocks(
			...
			function (): void {
				$request = new WP_REST_Request( 'POST', '/heckl/v1/following' );
	...
	private function test_websub_verification(): void {
		...
		$request = new WP_REST_Request( 'GET', '/heckl/v1/websub/callback' );
		...
		$request = new WP_REST_Request( 'POST', '/heckl/v1/websub/callback' );
	...
	private function test_rest_permissions_and_deletes(): void {
		...
		$response = $this->rest_request( 'DELETE', '/heckl/v1/following', [
			'type' => 'rss',
			'id'   => md5( $feed_url ),
			'url'  => $feed_url,
		] );
	 ...
	 *
	 *   - heckl/like-button     → core/post-template / last_child
	 *   - heckl/following-link  → core/navigation    / last_child
	 *   - heckl/favorites-link  → core/navigation    / last_child
	 *
	...
	private function test_hooked_block_insertions(): void {
		...
		$this->assert_same(
			...
			'heckl/following-link must be hooked into core/navigation/last_child exactly once.'
		);
		$this->assert_same(
			1,
			count( array_keys( $nav_hooks, 'heckl/favorites-link', true ) ),
			'heckl/favorites-link must be hooked into core/navigation/last_child exactly once.'
		);
		...
		$this->assert_same(
			...
			'heckl/like-button must NOT be added via the imperative hooked_block_types filter (block.json blockHooks is the canonical source).'
		);
		...
		$bt = WP_Block_Type_Registry::get_instance()->get_registered( 'heckl/like-button' );
		$this->assert_true(
			$bt && isset( $bt->block_hooks['core/post-template'] ),
			'heckl/like-button block.json must declare blockHooks for core/post-template.'
		);
		$this->assert_same(
			'last_child',
			$bt->block_hooks['core/post-template'] ?? null,
			'heckl/like-button block.json must declare its core/post-template hook at last_child.'
		);
		...
		$following = WP_Block_Type_Registry::get_instance()->get_registered( 'heckl/following-link' );
		$this->assert_true(
			$following && empty( $following->block_hooks ),
			'heckl/following-link must NOT declare blockHooks (imperative filter is the sole insertion path).'
		);
		$favorites = WP_Block_Type_Registry::get_instance()->get_registered( 'heckl/favorites-link' );
		$this->assert_true(
...
WP_CLI::add_command( 'heckl integration-tests', [ Radical_Socials_Integration_Tests::class, 'run' ] );
