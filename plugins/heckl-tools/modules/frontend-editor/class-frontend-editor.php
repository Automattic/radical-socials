 ...
 * Frontend Editor
 *
 * Registers the `heckl/frontend-editor` block and the
 * `heckl-frontend` script/style handles. block.json wires
...
class Radical_Socials_Frontend_Editor {
	...
	const SCRIPT_HANDLE = 'heckl-frontend';
	const STYLE_HANDLE  = 'heckl-frontend';
	...
	public static function render(): string {
		...
		return '<div id="heckl-editor"></div>';
	}
	...
	public static function maybe_inject_into_main( string $block_content, array $block ): string {
		...
		// enqueue time.
		$editor_html = do_blocks( '<!-- wp:heckl/frontend-editor /-->' );
		if ( '' === trim( $editor_html ) ) {
			return $block_content;
		}
	...
	private static function should_auto_inject(): bool {
		...
		foreach ( $templates as $tpl ) {
			if ( isset( $tpl->content ) && false !== strpos( (string) $tpl->content, 'wp:heckl/frontend-editor' ) ) {
				$cached = false;
				return $cached;
			}
		}
