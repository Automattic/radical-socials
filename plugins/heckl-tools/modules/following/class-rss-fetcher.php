...
class Radical_Socials_RSS_Fetcher {
	...
	public static function fetch( string $feed_url, int $count = 20 ): array {
		...
		if ( ! self::is_safe_remote_url( $feed_url ) ) {
			self::$last_error = __( 'URL is not a safe public address (private IP, localhost, or invalid format).', 'heckl-tools' );
			return [];
		}
	...
	private static function humanise_feed_error( WP_Error $error ): string {
		...
		if ( false !== strpos( $lc_raw, 'a feed could not be found at' ) ) {
			return __( 'The URL doesn\'t serve an RSS or Atom feed.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, '404 not found' ) || str_contains( $lc_raw, ' 404' ) ) {
			return __( 'The feed URL returns 404 Not Found — it may have moved or been removed.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, '403 forbidden' ) || str_contains( $lc_raw, ' 403' ) ) {
			return __( 'The feed server blocked our request (HTTP 403). The site may be rate-limiting or restricting feed access.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, '5' ) && ( str_contains( $lc_raw, '500' ) || str_contains( $lc_raw, '502' ) || str_contains( $lc_raw, '503' ) || str_contains( $lc_raw, '504' ) ) ) {
			return __( 'The feed server returned a 5xx error. The site is probably temporarily down.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, 'ssl' ) || false !== strpos( $lc_raw, 'certificate' ) ) {
			return __( 'The feed server\'s SSL certificate is invalid or expired.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, 'name or service not known' ) || false !== strpos( $lc_raw, 'could not resolve host' ) ) {
			return __( 'The feed\'s domain name doesn\'t resolve — the site may no longer exist.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, 'connection refused' ) ) {
			return __( 'The feed server refused the connection.', 'heckl-tools' );
		}
		if ( false !== strpos( $lc_raw, 'timed out' ) || false !== strpos( $lc_raw, 'timeout' ) ) {
			return __( 'The feed server didn\'t respond in time (timeout).', 'heckl-tools' );
		}
		if ( 'http_request_failed' === $code ) {
			return sprintf(
				/* translators: %s: low-level error message */
				__( 'Couldn\'t reach the feed server: %s', 'heckl-tools' ),
				$raw
			);
		}

		// Last resort: pass the raw message through but at least frame it.
		return $raw ?: __( 'Unknown feed error.', 'heckl-tools' );
	}
