		...
		foreach ( $actors as $post ) {
			...
			if ( ! $outbox_url ) {
				self::record_actor_health( $post->ID, [
					'status'     => 'failed',
					'error'      => __( 'Could not discover the actor\'s outbox URL (account may not exist on this server).', 'heckl-tools' ),
			...
			if ( ! $page ) {
				self::record_actor_health( $post->ID, [
					'status'     => 'failed',
					'error'      => __( 'The remote server returned an empty or malformed outbox page.', 'heckl-tools' ),
					'elapsed_ms' => (int) round( ( microtime( true ) - $poll_started ) * 1000 ),
				] );
				continue;
			}
	...
	private static function render_reply_context( array $object ): string {
		...
		$label = '' !== $display
			? sprintf(
				...
				__( '↪️ In reply to %s', 'heckl-tools' ),
