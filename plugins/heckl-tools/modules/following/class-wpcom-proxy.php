 ...
 *
 * Three endpoints (all under /wp-json/heckl/v1/wpcom-proxy/):
 *
...
class Radical_Socials_WPCOM_Proxy {
	...
	public static function handle_callback( WP_REST_Request $request ): void {
		...
		if ( ! self::looks_like_state( $state ) ) {
			wp_die( esc_html__( 'Bad OAuth state', 'heckl-tools' ), '', [ 'response' => 400 ] );
		}

		$origin = get_transient( self::STATE_PREFIX . $state );
		if ( ! $origin ) {
			wp_die( esc_html__( 'Unknown or expired OAuth state — start the connection again from your site.', 'heckl-tools' ), '', [ 'response' => 400 ] );
		}
		...
		// The destination is the consumer site we recorded at /init — a
		// validated (HTTPS-only, parseable) URL. wp_safe_redirect()
		// normally refuses cross-host hops, so we register that single
		// origin host with allowed_redirect_hosts just for this request.
		// Filter is removed immediately after so we never broaden the
		// allow-list for subsequent traffic.
		$origin_host = wp_parse_url( $origin, PHP_URL_HOST );
		$allow_host  = static function ( $hosts ) use ( $origin_host ) {
