...
const { state, actions } = store( 'heckl/following', {
	actions: {
		*refresh() {
			...
			try {
				...
				if ( ! res.ok ) {
					console.error( '[heckl] refresh failed:', res.status, res.statusText, { url: state.refreshUrl } );
				} else {
				...
				console.error( '[heckl] refresh network error:', err, { url: state.refreshUrl, nonce: !! state.nonce } );
			}
