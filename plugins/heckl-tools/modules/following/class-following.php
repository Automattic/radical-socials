...
class Radical_Socials_Following {
	...
	public static function register_blocks(): void {
		...
		register_block_type(
			...
			[ 'render_callback' => 'heckl_render_favorite_feeds' ]
		);
		register_block_type(
			__DIR__ . '/blocks/following-link',
			[ 'render_callback' => 'heckl_render_following_link' ]
		);
		register_block_type(
			__DIR__ . '/blocks/favorites-link',
			[ 'render_callback' => 'heckl_render_favorites_link' ]
		);
		...
		//   item).
		//
		// - `heckl/following-link` and `…/favorites-link` →
	...
	public static function hook_following_link( array $hooked_blocks, string $position, ?string $anchor_block ): array {
		if ( 'last_child' === $position && 'core/navigation' === $anchor_block ) {
			$hooked_blocks[] = 'heckl/following-link';
		}
	...
	public static function hook_favorites_link( array $hooked_blocks, string $position, ?string $anchor_block ): array {
		if ( 'last_child' === $position && 'core/navigation' === $anchor_block ) {
			$hooked_blocks[] = 'heckl/favorites-link';
		}
		return $hooked_blocks;
	}
	...
	public static function add_cron_schedules( array $schedules ): array {
		$schedules[ self::FETCH_SCHEDULE ] = [
			...
			'display'  => __( 'Every 15 minutes', 'heckl-tools' ),
		];
		return $schedules;
	}
	...
	public static function register_cpt(): void {
		register_post_type(
			...
			[
				...
				'labels'              => [
					'name'          => __( 'Following', 'heckl-tools' ),
					'singular_name' => __( 'Feed Item', 'heckl-tools' ),
				],
			]
		);
	}
	...
	public static function wrap_following_query( string $html, array $block ): string {
		...
		wp_interactivity_state( 'heckl/following', [
			...
			'refreshingLabel'      => __( 'Refreshing…', 'heckl-tools' ),
			'noNewPostsLabel'      => __( 'No new posts', 'heckl-tools' ),
			'stillRefreshingLabel' => __( 'Still refreshing', 'heckl-tools' ),
			'newPostLabel'         => translate_nooped_plural( $new_posts_forms, 1, 'heckl-tools' ),
		...
		if ( $processor->next_tag() ) {
			$processor->set_attribute( 'data-wp-interactive', 'heckl/following' );
	...
	private static function last_refreshed_label( int $last_fetched ): string {
		if ( ! $last_fetched ) {
			return __( 'Not refreshed yet', 'heckl-tools' );
		}

		$elapsed = time() - $last_fetched;
		if ( $elapsed < MINUTE_IN_SECONDS ) {
			return __( 'Last refreshed just now', 'heckl-tools' );
		}

		return sprintf(
			/* translators: %s: Human-readable elapsed time, for example "8 minutes". */
			__( 'Last refreshed %s ago', 'heckl-tools' ),
