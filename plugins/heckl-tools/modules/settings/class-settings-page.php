 ...
 * Settings Page
 *
 * Adds a Settings submenu under the Heckl top-level menu.
...
class Radical_Socials_Settings_Page {
	...
	public static function handle_diagnostics_action(): void {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=heckl-settings&tab=diagnostics' ) );
		exit;
	}
	...
	public static function handle_profile_save(): void {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=heckl-settings&tab=profile' ) );
		exit;
	}
	...
	private static function active_tab(): string {
		$tab     = heckl_url_param( 'tab' ) ?? 'profile';
	...
	public static function enqueue( string $hook ): void {
		if ( 'toplevel_page_heckl-settings' !== $hook ) {
			return;
		}
		...
		if ( in_array( self::active_tab(), [ 'following', 'welcome' ], true ) ) {
			...
			wp_localize_script( 'rs-following-settings', 'rsFollowing', [
				...
				'opmlParseUrl'        => rest_url( 'heckl/v1/following/opml/parse' ),
				'opmlEntryUrl'  => rest_url( 'heckl/v1/following/opml/entry' ),
				'opmlImportUrl' => rest_url( 'heckl/v1/following/opml/import' ),
				'opmlExportUrl' => add_query_arg( '_wpnonce', $nonce, rest_url( 'heckl/v1/following/opml/export' ) ),
				'nonce'         => $nonce,
				'i18n'          => [
					...
					'healthUntested' => __( 'Not yet checked. The status updates after the next refresh cycle reaches this feed.', 'heckl-tools' ),
					'healthUnknownError' => __( 'Unknown error', 'heckl-tools' ),
					'starLabel'     => __( 'Star this feed', 'heckl-tools' ),
					'unstarLabel'   => __( 'Unstar this feed', 'heckl-tools' ),
					'addSummary'    => __( 'Done — {added} added, {skipped} already existed, {failed} failed.', 'heckl-tools' ),
					'failuresLabel' => __( 'The following could not be added:', 'heckl-tools' ),
					'errorNetwork'  => __( 'Network error', 'heckl-tools' ),
					'errorUnknown'  => __( 'Unknown error', 'heckl-tools' ),
					'errors'        => [
						'invalid_url'                       => __( 'Invalid URL — make sure it starts with https://', 'heckl-tools' ),
						'unsafe_url'                        => __( 'URL must be a public HTTP(S) address', 'heckl-tools' ),
						'activitypub_unavailable'           => __( 'ActivityPub plugin is not active — install it to follow Fediverse accounts', 'heckl-tools' ),
						...
						'activitypub_user_not_found'        => __( 'This site is not set up to follow on the Fediverse yet — check the ActivityPub plugin\'s settings', 'heckl-tools' ),
						'activitypub_remote_actor_not_found' => __( 'Account not found — check the handle or URL', 'heckl-tools' ),
						'activitypub_already_following'     => __( 'Already following', 'heckl-tools' ),
						'already_exists'                    => __( 'Already following', 'heckl-tools' ),
					],
					'importAccountBtn'     => __( 'Import follows', 'heckl-tools' ),
					'importAccountFetching' => __( 'Fetching following list…', 'heckl-tools' ),
					'importAccountAdding'   => __( 'Adding {done} / {total}…', 'heckl-tools' ),
					'importAccountDone'     => __( 'Done — {added} added, {skipped} already existed, {failed} failed.', 'heckl-tools' ),
					'importAccountPrivate'  => __( 'This account\'s following list is private. Enable "Show following and followers publicly" in your Mastodon privacy settings and try again.', 'heckl-tools' ),
					'importAccountNotFound' => __( 'Account not found. Check the handle and try again.', 'heckl-tools' ),
					'importAccountError'    => __( 'Could not fetch following list. Try again.', 'heckl-tools' ),
					'importBtn'     => __( 'Import OPML', 'heckl-tools' ),
					'importing'     => __( 'Importing…', 'heckl-tools' ),
					'importResult'  => __( 'Done — {added} added, {updated} updated, {skipped} unchanged, {failed} failed.', 'heckl-tools' ),
					'importError'   => __( 'Import failed. Make sure the file is a valid OPML document.', 'heckl-tools' ),
					'importNoFile'  => __( 'Please choose an OPML file first.', 'heckl-tools' ),
					'feedsHeading'  => __( 'Feeds ({count})', 'heckl-tools' ),
				],
			] );
		}
	}
	...
	public static function render(): void {
		...
		$bio_empty   = __( 'Add a short bio so people know what to expect from your profile.', 'heckl-tools' );
		...
		<div class="wrap rs-settings-wrap">
				...
				<div class="rs-settings-tabs-scroller">
					<nav class="rs-settings-tabs-wrapper" aria-label="<?php esc_attr_e( 'Settings sections', 'heckl-tools' ); ?>">
						...
						$tabs = [
							...
							'profile'   => __( 'Profile', 'heckl-tools' ),
							'following' => __( 'Following', 'heckl-tools' ),
						];
			...
			<?php if ( ! empty( $notice['saved'] ) ) : ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'Settings saved.', 'heckl-tools' ); ?></p>
				</div>
			<?php endif; ?>

			<?php if ( ! empty( $notice['oauth_connected'] ) ) : ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'WP.com account connected. Your following feed will populate shortly.', 'heckl-tools' ); ?></p>
				</div>
			<?php endif; ?>

			<?php if ( ! empty( $notice['wpcom_disconnected'] ) ) : ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'WP.com account disconnected.', 'heckl-tools' ); ?></p>
				</div>
			<?php endif; ?>
			...
			<?php if ( ! empty( $notice['oauth_error'] ) ) :
				$oauth_error_msgs = [
					...
					'token_exchange_failed' => __( 'Could not exchange the WordPress.com code for an access token. Try again.', 'heckl-tools' ),
					'no_token'              => __( 'WordPress.com returned an unexpected response (no access token). Try again.', 'heckl-tools' ),
				];
				$oauth_error_msg = $oauth_error_msgs[ $notice['oauth_error'] ] ?? __( 'Could not connect WP.com account. Please try again.', 'heckl-tools' );
				?>
							...
							<div class="rs-field hide-if-no-js site-icon-section">
								...
								<div class="site-icon-action-buttons">
									<button
										...
										data-size="512"
										data-choose-text="<?php esc_attr_e( 'Select Logo', 'heckl-tools' ); ?>"
										data-update-text="<?php esc_attr_e( 'Change Logo', 'heckl-tools' ); ?>"
										data-update="<?php esc_attr_e( 'Use this logo', 'heckl-tools' ); ?>"
										data-state="<?php echo esc_attr( $has_logo ? '1' : '' ); ?>"
									>
										<?php echo $has_logo
											? esc_html__( 'Change Logo', 'heckl-tools' )
											: esc_html__( 'Select Logo', 'heckl-tools' ); ?>
									</button>
									<button
									...
									>
										<?php esc_html_e( 'Remove Logo', 'heckl-tools' ); ?>
									</button>
								</div>
							</div>
							<?php endif; ?>
				...
				<div>
					...
					<form method="post" style="margin:16px 0 24px;padding:12px 14px;border:1px solid #dcdcde;border-radius:4px;background:#f6f7f7">
						...
						<p style="margin:10px 0 0">
							<button type="submit" class="button button-secondary"><?php esc_html_e( 'Save', 'heckl-tools' ); ?></button>
						</p>
					</form>
	...
	private static function render_welcome_tab(): void {
		...
		<?php if ( $ap_installed ) : ?>
			<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'ActivityPub installed and activated.', 'heckl-tools' ); ?></p></div>
		<?php endif; ?>
		<?php if ( $ap_error ) : ?>
			<div class="notice notice-error is-dismissible"><p><?php
				printf(
					/* translators: %s: error message from the WP plugin installer */
					esc_html__( 'Could not install ActivityPub: %s', 'heckl-tools' ),
					esc_html( $ap_error )
				);
			?></p></div>
		<?php endif; ?>
			...
			<li class="<?php echo esc_attr( $step_class( 2 ) ); ?>">
				...
				<div class="rs-welcome-step__body">
					...
					<div class="rs-welcome-step__addbox">
						...
						<p class="description" style="margin:4px 0 8px">
							<?php if ( $ap_active ) : ?>
								<?php esc_html_e( 'One per line. RSS/Atom URLs or ActivityPub handles (e.g. @someone@mastodon.social).', 'heckl-tools' ); ?>
							<?php else : ?>
					...
					<div class="rs-welcome-step__addbox">
						...
						<?php if ( Radical_Socials_WPCOM_OAuth::is_connected() ) : ?>
							<span style="color:#00a32a">✓ <?php esc_html_e( 'Connected.', 'heckl-tools' ); ?></span>
						<?php else : ?>
							<a href="<?php echo esc_url( Radical_Socials_WPCOM_OAuth::connect_url() ); ?>" class="button button-secondary"><?php esc_html_e( 'Connect WordPress.com', 'heckl-tools' ); ?></a>
						<?php endif; ?>
					</div>
					<?php endif; ?>
	...
	public static function handle_install_activitypub(): void {
		...
		if ( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin_file ) ) {
			...
			if ( ! $result ) {
				...
				$back( $messages ? implode( '; ', $messages ) : __( 'Installer returned no result.', 'heckl-tools' ) );
			}
		}
	...
	private static function render_diagnostics_tab( string $result = '' ): void {
		...
		if ( $last === 0 && ( $rss_count + $ap_count ) > 0 ) {
			$warnings[] = [ 'level' => 'error', 'msg' => __( 'Feeds have never refreshed despite having active subscriptions. Use "Run fetch now" below to do a one-time inline run.', 'heckl-tools' ) ];
		} elseif ( $last > 0 && ( time() - $last ) > HOUR_IN_SECONDS ) {
			$warnings[] = [ 'level' => 'warning', 'msg' => __( 'The last successful refresh was over an hour ago. Cron may not be running on your host.', 'heckl-tools' ) ];
		}
		if ( $cron_disabled ) {
			$warnings[] = [
				'level' => 'info',
				'msg'   => __( 'WordPress\'s built-in cron is disabled on this site (DISABLE_WP_CRON = true). The 15-minute background refresh only runs if your host has a real cron job configured to hit wp-cron.php. Hostinger users: set this up in hPanel → Cron Jobs with the command shown below.', 'heckl-tools' ),
			];
		}
		if ( Radical_Socials_Following::REFRESH_LOCK_QUEUED === $lock ) {
			$warnings[] = [ 'level' => 'warning', 'msg' => __( 'A refresh is currently queued. If this state persists for more than a couple of minutes, cron isn\'t firing — clear the lock and try "Run fetch now".', 'heckl-tools' ) ];
		}
		if ( $mem_bytes > 0 && $mem_bytes < 128 * MB_IN_BYTES ) {
			$warnings[] = [
				'level' => 'warning',
				/* translators: %s: PHP memory_limit ini value (e.g. "64M"). */
				'msg'   => sprintf( __( 'PHP memory_limit is %s. Polling 100+ feeds may run out of memory. Raise it to at least 128M.', 'heckl-tools' ), ini_get( 'memory_limit' ) ),
			];
		}
		if ( $exec_time > 0 && $exec_time < 60 ) {
			$warnings[] = [
				...
				'msg'   => sprintf( __( 'PHP max_execution_time is %ds. A full refresh of many feeds may not finish before the host kills it.', 'heckl-tools' ), $exec_time ),
			];
		}
		if ( ! function_exists( 'Activitypub\follow' ) ) {
			$warnings[] = [
				...
				'msg'   => sprintf(
					/* translators: %s: link to the ActivityPub plugin install screen */
					__( 'The ActivityPub plugin is not active. RSS and WP.com Reader follows work without it; install %s to also follow Fediverse accounts and federate your own posts.', 'heckl-tools' ),
					'<a href="' . esc_url( self_admin_url( 'plugin-install.php?s=activitypub&tab=search&type=term' ) ) . '">' . esc_html__( 'ActivityPub', 'heckl-tools' ) . '</a>'
				),
			];
		}
		...
		<div style="margin-top:20px;max-width:840px">
			...
			<h2 style="margin-top:24px"><?php esc_html_e( 'Feed refresh status', 'heckl-tools' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Last successful refresh', 'heckl-tools' ); ?></th>
					<td><?php
						if ( $last === 0 ) {
							esc_html_e( 'Never', 'heckl-tools' );
						} else {
							echo esc_html( sprintf(
								/* translators: 1: date 2: time-diff */
								__( '%1$s (%2$s ago)', 'heckl-tools' ),
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Refresh lock', 'heckl-tools' ); ?></th>
					<td><code><?php echo $lock === false ? esc_html__( '(none)', 'heckl-tools' ) : esc_html( (string) $lock ); ?></code></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Next scheduled refresh', 'heckl-tools' ); ?></th>
					<td><?php
						if ( ! $next_ts ) {
							esc_html_e( 'Not scheduled', 'heckl-tools' );
						} else {
							echo esc_html( sprintf(
								/* translators: 1: date 2: schedule 3: diff */
								__( '%1$s — schedule %2$s (in %3$s)', 'heckl-tools' ),
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Subscriptions', 'heckl-tools' ); ?></th>
					<td><?php
						if ( function_exists( 'Activitypub\\follow' ) ) {
							echo esc_html( sprintf(
								/* translators: 1: RSS count 2: AP count */
								__( '%1$d RSS, %2$d ActivityPub', 'heckl-tools' ),
								$rss_count,
								$ap_count
							) );
						} else {
							echo esc_html( sprintf(
								...
								_n( '%d RSS feed', '%d RSS feeds', $rss_count, 'heckl-tools' ),
								$rss_count
							) );
						}
					?></td>
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Feed items stored', 'heckl-tools' ); ?></th>
					<td><?php echo esc_html( (string) $item_total ); ?></td>
				</tr>
			</table>

			<h2 style="margin-top:24px"><?php esc_html_e( 'Actions', 'heckl-tools' ); ?></h2>
			...
			<form method="post" style="display:inline-block;margin-right:8px">
				...
				<button type="submit" class="button button-secondary"<?php disabled( 0 === ( $rss_count + $ap_count ) || 0 === $feed_remaining ); ?>>
					<?php
					if ( 0 === $feed_tested ) {
						esc_html_e( 'Test all feeds', 'heckl-tools' );
					} else {
						printf(
							/* translators: 1: tested count, 2: total target count */
							esc_html__( 'Test next batch (%1$d / %2$d tested)', 'heckl-tools' ),
							(int) $feed_tested,
							(int) $feed_total
						);
					}
			...
			<?php if ( $feed_tested > 0 ) :
					...
					<?php if ( ! empty( $feed_test['last_run'] ) ) : ?>
						...
						<span class="description"><?php
							printf(
								...
								esc_html__( 'last batch %s ago', 'heckl-tools' ),
				...
				<p class="description" style="margin-top:-6px">
					...
					printf(
						...
						esc_html__( '%1$d / %2$d RSS · %3$d / %4$d ActivityPub tested', 'heckl-tools' ),
						(int) $by_type['rss'],
				...
				<table class="widefat striped" style="max-width:840px">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Status', 'heckl-tools' ); ?></th>
							<th><?php esc_html_e( 'Time', 'heckl-tools' ); ?></th>
							<th><?php esc_html_e( 'Bytes', 'heckl-tools' ); ?></th>
							<th><?php esc_html_e( 'Type', 'heckl-tools' ); ?></th>
							<th><?php esc_html_e( 'URL / error', 'heckl-tools' ); ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<h2 style="margin-top:32px"><?php esc_html_e( 'Hosting', 'heckl-tools' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row">DISABLE_WP_CRON</th>
					<td><code><?php echo $cron_disabled ? 'true' : 'false'; ?></code>
						<?php if ( $cron_disabled ) : ?>
							<p class="description"><?php esc_html_e( 'WordPress will not auto-fire scheduled events on page visits. A real cron job on your host must call wp-cron.php instead. Recommended command:', 'heckl-tools' ); ?></p>
							...
							<p class="description"><?php esc_html_e( 'Schedule this every 15 minutes (cron expression: */15 * * * *).', 'heckl-tools' ); ?></p>
						<?php endif; ?>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><?php echo function_exists( 'Activitypub\follow' ) ? '<span style="color:#0a7b3f">✓ ' . esc_html__( 'active', 'heckl-tools' ) . '</span>' : '<span style="color:#dc3232">✘ ' . esc_html__( 'not active', 'heckl-tools' ) . '</span>'; ?></td>
				</tr>
			</table>
		</div>
		<?php
		...
		switch ( $result ) {
			case 'fetch_ran':
				$body = sprintf(
					...
					esc_html__( 'Refresh completed in %ss.', 'heckl-tools' ),
					esc_html( (string) $elapsed )
				);
				break;
			case 'fetch_error':
				$class = 'notice notice-error is-dismissible';
				$body  = esc_html__( 'Inline refresh threw an exception:', 'heckl-tools' ) . ' <code>' . esc_html( (string) $err_msg ) . '</code>';
				break;
			case 'queued':
				$body = esc_html__( 'Refresh queued. Watch the "Last successful refresh" timestamp — it should update within a minute or two.', 'heckl-tools' );
				break;
			case 'queue_failed':
				$class = 'notice notice-warning is-dismissible';
				$body  = esc_html__( 'Refresh could not be queued (another fetch is already running).', 'heckl-tools' );
				break;
			case 'lock_cleared':
				$body = esc_html__( 'Refresh lock cleared.', 'heckl-tools' );
				break;
			case 'no_subs':
				$class = 'notice notice-warning is-dismissible';
				$body  = esc_html__( 'No RSS subscriptions to test against.', 'heckl-tools' );
				break;
			case 'feed_test_batch':
				$body = esc_html__( 'Tested a batch of feeds — see the results table below. Click again to test the next batch.', 'heckl-tools' );
				break;
			case 'feed_test_empty':
				$class = 'notice notice-info is-dismissible';
				$body  = esc_html__( 'Every subscription has already been tested. Use "Reset feed-test results" to start over.', 'heckl-tools' );
				break;
			case 'feed_test_reset':
				$body = esc_html__( 'Feed-test results cleared.', 'heckl-tools' );
				break;
			case 'tested':
				if ( is_array( $test ) && ! empty( $test['ok'] ) ) {
					$body = sprintf(
						/* translators: 1: URL 2: HTTP code 3: bytes */
						esc_html__( 'Fetched %1$s — HTTP %2$d, %3$d bytes.', 'heckl-tools' ),
					...
					$body  = sprintf(
						...
						esc_html__( 'Could not fetch %1$s — %2$s', 'heckl-tools' ),
