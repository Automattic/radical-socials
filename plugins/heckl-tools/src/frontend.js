...
modalRoot.id = 'heckl-modal-root';
document.body.appendChild( modalRoot );
createRoot( modalRoot ).render( <EditorModal /> );

// Mount inline editor only if the theme provides the mount point
const inlineRoot = document.getElementById( 'heckl-editor' );
if ( inlineRoot ) {
	createRoot( inlineRoot ).render( <FrontendEditor /> );
}

// Wire custom bar Create button (and any future [data-heckl-action="open-editor"] elements)
document.addEventListener( 'click', ( e ) => {
	const trigger = e.target.closest( '[data-heckl-action="open-editor"]' );
	if ( trigger ) {
		e.preventDefault();
